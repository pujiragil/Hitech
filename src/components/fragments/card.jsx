...
const Card = ({ order, primary, textColor, buttonText, buttonBg, textWhite, buttonHov, buttonBorderHov }) => {
  return (
    <div className={`${ primary ? 'bg-white' : 'bg-zinc-900' } lg:w-full md:w-3/4 w-full ${order} ${textColor} shadow-2xl rounded-large flex flex-col p-6 mb-6`}>
        ...
        <Link to="hero" spy={true} smooth={true} duration={700} className={`${buttonBg} hover:${buttonHov} cursor-pointer border-2 border-transparent ${buttonBorderHov} text-center duration-300 ${buttonText} hover:${textWhite} text-sm font-medium py-3 rounded-full mb-6 drop-shadow-xl`}>Buy now</Link>          
